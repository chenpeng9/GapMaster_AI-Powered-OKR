    ...
    <>
      <div className="mb-10">
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
          <span className="text-primary">+</span> 新建 Objective
        </h2>
        <form onSubmit={onCreateObjective} className="flex flex-col gap-4 sm:flex-row">
          <div className="flex-1">
            <Label htmlFor="objective-title" className="text-xs mb-1.5 text-muted-foreground">
              Objective 标题
            </Label>
            <Input
              id="objective-title"
              ...
              required
              className="rounded-lg border-border/60 focus:border-primary focus:ring-primary/20"
            />
          </div>
          <div>
            <Label htmlFor="objective-quarter" className="text-xs mb-1.5 text-muted-foreground">
              季度
            </Label>
            <Select value={newObjectiveQuarter} onValueChange={onNewObjectiveQuarterChange}>
              <SelectTrigger className="w-[140px] bg-background rounded-lg border-border/60 focus:border-primary" id="objective-quarter">
          ...
          <div className="flex items-end">
            <Button className="w-full rounded-lg transition-all hover:shadow-md" type="submit" disabled={disableObjectiveBtn}>
              {creatingObjective ? "创建中..." : "新建 Objective"}
            </Button>
          </div>
        </form>
      </div>
      ...
      <div className="mb-10">
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
          <span className="text-primary">+</span> 添加 Key Result
        </h2>
      ...
      >
        <DialogContent className="sm:max-w-md rounded-2xl border-border/60">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold">编辑内容</DialogTitle>
          </DialogHeader>
          {editData && (
            <form
              onSubmit={async (e) => {
            ...
            >
              <div className="flex flex-col gap-4 py-2">
                <div>
                  <Label className="text-xs mb-1.5 text-muted-foreground" htmlFor="edit-title">
                    标题
                  </Label>
                  <Input
                    id="edit-title"
                ...
                {editData.type === "KR" && (
                  <div>
                    <Label className="text-xs mb-1.5 text-muted-foreground" htmlFor="edit-target-value">
                      目标值
                    </Label>
                    <Input
              ...
              <DialogFooter className="gap-2 mt-6">
                <DialogClose asChild>
                  <Button type="button" variant="outline" disabled={editSaving} className="rounded-lg">
                    取消
                  </Button>
                </DialogClose>
                <Button
      ...
      <div>
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
          ...
          当前 Objectives & Key Results
        </h2>
        {okrLoading ? (
          <FancyLoader text="加载中..." />
        ) : okrError ? (
          <div className="p-4 text-destructive bg-destructive/10 rounded-lg border border-destructive/20">{okrError}</div>
        ) : (
          <div className="flex flex-col space-y-4">
            {objectivesList.length === 0 ? (
              <div className="text-sm text-muted-foreground py-12 text-center bg-muted/20 rounded-xl border border-dashed border-border/40">
                <div className="flex flex-col items-center gap-2">
                  <Target className="w-10 h-10 text-muted-foreground/30" />
                  <p>暂无 OKR，请先创建 Objective</p>
                </div>
              </div>
            ) : (
